...
class App {
  ...
  public function hookFormAlter(&$form, FormStateInterface $form_state, string $form_id) {
    try {
      ...
      \Drupal::messenger()->addMessage(t('An error occurred in webform_submission_change_history: @t at @f:@l. See the log for more details.', [
        '@t' => $t->getMessage(),
        '@f' => $t->getFile(),
        '@l' => $t->getLine(),
      ]), 'error');
  ...
  public function hookRequirements(string $phase) : array {
    ...
    if ($phase == 'runtime') {
      ...
      $requirements['webform_submission_change_history_patch'] = [
  ...
  public function webformIsPatched() : bool {
    ...
    $contents = file_get_contents($module_path . '/src/Entity/WebformSubmission.php');
